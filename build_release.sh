...
function main() {
  ...
  PACKAGE="github.com/rverst/miabdns/cmd/miabdns-server"
  BUILDDATE=$(date -u -Iseconds)
  LDFLAGS="-X $PACKAGE/main.Version=$VERSION -X $PACKAGE/main.CommitHash=$COMHASH -X $PACKAGE/main.BuildDate=$BUILDDATE"

  DIR=$PWD
  BUILDDIR="${DIR}/build/${VERSION}"
  CHECKSUM="${BUILDDIR}/miabdns_${VERSION}_checksums.txt"
  LICENSE="${DIR}/LICENSE.txt"
  README="${DIR}/README.md"
  CONFIG="${DIR}/example_config.json"
  ...
  for GOOS in windows darwin linux; do
    for GOARCH in 386 amd64; do
      ...
      OSARCH="miabdns_${GOOS}-${GOARCH}"
      OSARCH_BUILDDIR="${BUILDDIR}/${OSARCH}"
      FILE="${OSARCH_BUILDDIR}/miabdns${EXT}"

      export GOOS GOARCH
      mkdir -p "$OSARCH_BUILDDIR"

      go build -v -ldflags "$LDFLAGS" -o "$FILE" ./cmd/miabdns-server/main.go

      cd "$BUILDDIR" || exit 1

      cp "$LICENSE" "${OSARCH_BUILDDIR}/LICENSE.txt"
      cp "$README" "${OSARCH_BUILDDIR}/README.md"
      cp "$CONFIG" "${OSARCH_BUILDDIR}/example_config.json"

      if [[ $GOOS == "windows" ]]; then
        AFILE="miabdns_${VERSION}_${GOOS}-${GOARCH}.zip"
        ARCHIVE="${BUILDDIR}/${AFILE}"
        zip -r "$ARCHIVE" "$OSARCH"
        sha256sum "$AFILE" >>"$CHECKSUM"

      else
        AFILE="miabdns_${VERSION}_${GOOS}-${GOARCH}.tar.gz"
        ARCHIVE="${BUILDDIR}/${AFILE}"
